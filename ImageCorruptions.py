...
def gaussian_noise(x, severity=1):
    ...
    x = np.array(x)
    return np.clip(x + np.random.normal(size=x.shape, scale=c), -1.8816435, 2.0934134) 
...
def shot_noise(x, severity=1):
    ...
    print(x.min())
    x = np.array(x)/ 255.
    return ((np.clip(np.random.poisson(x * c) / float(c), 0, 1) * 255)-mean-1.5258789e-05)/(std+1e-7)
...
def brightness(x, severity=1):
    ...
    x[:, :, :, 2] = np.clip(x[:, :, :, 2] + c, -1.8816435, 2.0934134)
...
def snow(x, severity=1):
    ...
    snow_layer = PILImage.fromarray((np.clip(snow_layer.squeeze(), 0, 1)).astype(np.uint8), mode='L')
    output = BytesIO()
...
def pixelate(x, severity=1):
    ...
    for c in range(x.shape[0]):
        im = PIL.Image.fromarray(x[c])
        print(im.size)
        im = im.resize((int(224 * c), int(224 * c)), PILImage.BOX)
        im = im.resize((224, 224), PILImage.BOX)
...
def defocus_blur(x, severity=1):
    ...
    for i in range(x.shape[0]):
        ...
        ims.append(np.clip(channels,-1.8816435, 2.0934134) ) #* 255 -1.8816435, 2.0934134
    return np.stack(ims,0)
